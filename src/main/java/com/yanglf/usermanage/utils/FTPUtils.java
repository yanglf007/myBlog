...
public class FTPUtils {
     ...
     */
    public static boolean uploadLocalFile(InputStream fis, String savePath, String fileName,
                                   String remoteIP,String ftpUser,String ftpPwd) {
        // 登录
        if(ftpClient==null){
            login(remoteIP, FTP_PORT, ftpUser, ftpPwd);
        }
        boolean flag = false;
        if (ftpClient != null) {
            try {
                ...
                // 上传文件
                flag = ftpClient.storeFile(new String(savePath.getBytes(localCharset), serverCharset), fis);
            } catch (Exception e) {
